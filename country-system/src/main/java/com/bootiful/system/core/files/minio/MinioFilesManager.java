package com.bootiful.system.core.files.minio;
...
public class MinioFilesManager {
     ...
     * @return 上传 返回结果
     */
    public ObjectWriteResponse putObject(String fileName, InputStream inputStream, long objectSize, long partSize) {
    ...
    public Mono<Map<String, Object>> getFormDataUrl(UploadUrlRequest uploadUrlRequest) {
        ...
        return Mono.defer(() -> Mono.just(Map.of("url", minioProperties.getEndpoint() + "/" + bucketName,
                "fileName", objectName, "formData", formData,
                "getUrl", minioProperties.getEndpoint() + "/" + bucketName + "/" + objectName)));
    }
