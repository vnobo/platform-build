...
comment on column sys_custom_menus.user_id is '用户登录信息ID';
comment on column sys_custom_menus.menu_id is '菜单id';
comment on column sys_custom_menus.name is '菜单名称';
comment on column sys_custom_menus.path is '菜单路径';
comment on column sys_custom_menus.sort is '菜单排序';
comment on column sys_custom_menus.extend is '扩展JSON字段';
comment on table sys_custom_menus is '用户自定义菜单.';
...
comment on column sys_configurations.tenant_id is '租户ID';
comment on column sys_configurations.name is '配置名称';
comment on column sys_configurations.configuration is '配置项JSON字段';
comment on table sys_configurations is '租户系统配置表.';
...
comment on column sys_user_configurations.tenant_id is '租户ID';
comment on column sys_user_configurations.system is '系统类型';
comment on column sys_user_configurations.type is '配置类型';
comment on column sys_user_configurations.configuration is '配置项JSON字段';
comment on table sys_user_configurations is '用户系统配置表';
...
comment on column sys_villages.code is '村庄行政编码';
comment on column sys_villages.street_code is '乡级（乡镇、街道）行政编码';
comment on column sys_villages.province_code is '省级（省份、直辖市、自治区）行政编码';
comment on column sys_villages.city_code is '地级（城市）行政编码';
comment on column sys_villages.area_code is '县级（区县）行政编码';
comment on table sys_villages is '全国村级行政记录表.';
...
comment on column sys_news.system is '新闻系统类型';
comment on column sys_news.tenant_id is '租户id';
comment on column sys_news.creator is '创建人';
comment on column sys_news.updater is '修改人';
comment on column sys_news.title is '标题';
comment on column sys_news.type is '新闻类型';
comment on column sys_news.status is '状态-审核用';
comment on column sys_news.content is '新闻内容';
comment on column sys_news.source is '新闻来源';
comment on column sys_news.auditor is '审核人id';
comment on column sys_news.audit_time is '审核时间';
comment on column sys_news.content is '新闻内容';
comment on column sys_news.created_time is '创建时间';
comment on column sys_news.updated_time is '最后一次修改时间';
comment on column sys_news.extend is '预留字段';
comment on table sys_news is '新闻';
...
comment on column sys_notifications.id is '主键';
comment on column sys_notifications.tenant_id is '租户id';
comment on column sys_notifications.tenant_code is '租户code';
comment on column sys_notifications.type is '1、公告 2、通知';
comment on column sys_notifications.title is '行政监督标题';
comment on column sys_notifications.content is '行政监督内容';
comment on column sys_notifications.created_time is '创建时间';
comment on column sys_notifications.updated_time is '最后一次修改时间';
comment on column sys_notifications.extend is '预留字段';
comment on column sys_notifications.creator is '创建人';
comment on column sys_notifications.updater is '修改人';
comment on table sys_notifications is '通知公告';
...
comment on column sys_common_problem.tenant_id is '租户ID';
comment on column sys_common_problem.title is '问题标题';
comment on column sys_common_problem.solution is '解决方法';
comment on column sys_common_problem.system is '系统类型';
comment on column sys_common_problem.extend is '保留字段';
comment on table sys_common_problem is '常见问题.';
