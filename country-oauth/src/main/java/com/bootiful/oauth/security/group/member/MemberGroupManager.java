...
import com.bootiful.oauth.security.user.UserOnly;
import com.bootiful.oauth.security.user.UserRepository;
...
public class MemberGroupManager extends BaseAutoToolsUtil {
    ...
    public Mono<Page<MemberGroupOnly>> members(MemberGroupRequest groupRequest, Pageable pageable) {
        ...
        String querySql = "select se_group_members.id,se_group_members.group_id,se_group_members.user_id," +
        ...
        String countSql = "select count(*) from se_group_members" +
                " inner join se_users on se_group_members.user_id = se_users.id" +
                " inner join se_groups on se_group_members.group_id = se_groups.id " +
                groupRequest.toMemberWhere();

        Flux<MemberGroupOnly> queryFlux = super.entityTemplate.getDatabaseClient()
                .sql(querySql + SqlUtils.applyPage(pageable))
                .map((row, rowMetadata) -> super.mappingR2dbcConverter.read(MemberGroupOnly.class, row, rowMetadata))
                .all();
        Mono<Long> countMono = super.entityTemplate.getDatabaseClient()
                .sql(countSql)
                .map((row, rowMetadata) -> super.mappingR2dbcConverter.read(Long.class, row, rowMetadata))
                .first();

        return queryFlux.collectList().zipWith(countMono)
    ...
    public Flux<MemberGroupOnly> addMembers(Integer groupId, Set<Long> userList) {
        return Flux.fromStream(userList.parallelStream()).map(userId -> MemberGroupRequest.of(groupId, userId))
                .flatMap(this::save).flatMapSequential(this::integrateOnly);
    }
    ...
    public Mono<MemberGroup> save(MemberGroup group) {
        return this.memberRepository.findByGroupIdAndUserId(group.getGroupId(), group.getUserId())
                .switchIfEmpty(this.memberRepository.save(group));
    }
    /**
     * 从 MemberGroup 转换为 only,
     * 1,增加角色信息
     * 2,增加用户信息
     *
    ...
    private Mono<MemberGroupOnly> integrateOnly(MemberGroup memberGroup) {
        return this.userRepository.findById(memberGroup.getUserId())
                .zipWith(this.groupManager.loadGroupOnly(memberGroup.getGroupId()))
                .map(entityTuples -> MemberGroupOnly.withMemberGroup(memberGroup)
