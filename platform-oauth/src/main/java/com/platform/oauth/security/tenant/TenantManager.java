...
public class TenantManager extends BaseAutoToolsUtil {
    ...
    public Mono<Page<Tenant>> page(TenantRequest tenantRequest, Pageable pageable) {
        return search(tenantRequest, pageable).collectList()
                .zipWith(super.entityTemplate.getDatabaseClient().sql(tenantRequest.toCountSql())
                        .map(row -> mappingR2dbcConverter.read(Long.class, row)).one())
    ...
    public Mono<Tenant> save(Tenant tenant) {
        if (tenant.isNew()) {
            ...
            return this.tenantRepository.findById(tenant.getId())
                    .flatMap(old -> {
