package com.bootiful.oauth.security.group;

import com.bootiful.commons.security.ReactiveSecurityDetailsHolder;
import com.bootiful.oauth.security.group.authority.AuthorityGroup;
import com.bootiful.oauth.security.group.authority.AuthorityGroupRequest;
import com.bootiful.oauth.security.group.member.MemberGroupManager;
import com.bootiful.oauth.security.group.member.MemberGroupOnly;
import com.bootiful.oauth.security.group.member.MemberGroupRequest;
...
public class GroupController {
    ...
    public Flux<AuthorityGroup> authorizing(@PathVariable Integer id, @RequestBody AuthorityGroupRequest request) {
    ...
    public Mono<Page<MemberGroupOnly>> members(@Parameter(name = "角色组ID") @PathVariable Integer id,
                                               @Parameter(name = "获取角色查询条件") MemberGroupRequest groupRequest,
                                               @PageableDefault(sort = "se_group_members.user_id", direction = Sort.Direction.DESC) Pageable pageable) {
        return this.memberGroupManager.members(groupRequest.id(id), pageable);
    }

    @Operation(summary = "获取角色组的用户")
    @GetMapping("members")
    public Mono<Page<MemberGroupOnly>> members(MemberGroupRequest groupRequest,
                                               @PageableDefault(sort = "se_group_members.user_id", direction = Sort.Direction.DESC) Pageable pageable) {
