package com.bootiful.oauth.security.group.authority;
...
public class AuthorityGroupManger extends BaseAutoToolsUtil {
    ...
    public Flux<AuthorityGroup> authorizing(Integer groupId, AuthorityGroupRequest authorityList) {
        return this.authorityGroupRepository.findByGroupId(groupId)
                .collectList().flatMapMany(oldList -> {
                    List<AuthorityGroup> addList = authorityList.getRules().parallelStream()
                    ...
                    List<AuthorityGroup> deleteList = oldList.parallelStream()
                            ...
                            .collect(Collectors.toList());
                    return this.authorityGroupRepository.saveAll(addList)
                            .publishOn(Schedulers.boundedElastic())
                            .doOnComplete(() -> this.authorityGroupRepository.deleteAll(deleteList).subscribe());
                });
    }
