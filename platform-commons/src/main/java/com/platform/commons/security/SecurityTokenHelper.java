...
/**
 * com.bootiful.commons.security.SecurityDetailsDesc
 * 安全认证token工具类
 *
...
public class SecurityTokenHelper {
    ...
    public final static String SECURITY_TOKEN_CONTEXT = "COUNTRY_SECURITY_DETAILS_CONTEXT";
     ...
     * @return 登录后的 token
     */
    public static Mono<AuthenticationToken> authenticationTokenMono(ServerWebExchange exchange,
                                                                    Authentication authentication) {

        return exchange.getSession().doOnNext(session -> session.getAttributes()
    ...
    public static UserDetails buildUserDetails(LoginSecurityDetails securityDetails) {
        return org.springframework.security.core.userdetails.User.withUsername(securityDetails.getUsername())
    ...
    public static Mono<Void> saveToken(ServerWebExchange exchange, SecurityDetails securityDetails) {
        return exchange.getSession().doOnNext((session) -> putToken(session.getAttributes(), securityDetails))
