package com.platform.oauth.utils;
...
public class InitializingTenantUtils extends BaseAutoToolsUtil {
  ...
  public InitializingTenantUtils(
        ...
        builder
            .codecs(
                clientCodecConfigurer ->
                    clientCodecConfigurer.defaultCodecs().maxInMemorySize(1024 * 1024 * 1024))
            .build();
                ...
                Flux.fromStream(
                    StreamSupport.stream(
                        Spliterators.spliteratorUnknownSize(
                            jsonNode.elements(), Spliterator.ORDERED),
                        true)))
        .filter(item -> item.get("code").asText().equals(code))
        .flatMap(
            item -> {
              ...
              return this.initializingTenant(
                      VillagesData.builder()
                          .codes(codes)
                          .addresses(addresses)
                          .pTenant(TenantRequest.withId(0))
                          .item(item)
                          .build())
                  .publishOn(Schedulers.boundedElastic())
                  .transformDeferredContextual(
                      (tenantMono, contextView) ->
                          tenantMono.doOnNext(
                              tenant ->
                                  this.recursiveInitialization(
                                      tenant, item.withArray("children"), contextView)));
            });
  }
   ...
   */
  private void recursiveInitialization(
      Tenant pTenant, ArrayNode childNode, ContextView contextView) {
    hotFlux
        .flatMap(this::initializingTenant)
        .subscribe(
            result ->
                log.info(
                    "村村村村村村村村村: id:{}, name: {}, code: {}",
                    result.getId(),
                    result.getName(),
                    result.getCode()));
    this.recursiveExpand(pTenant, childNode)
        .delayUntil(this::recursiveVillages)
  ...
  private Flux<Tenant> recursiveExpand(Tenant pTenant, ArrayNode childNode) {
    return Flux.fromStream(
            StreamSupport.stream(
                Spliterators.spliteratorUnknownSize(childNode.iterator(), Spliterator.ORDERED),
                true))
        .flatMap(
            item -> {
              List<String> codes =
                  objectMapper.convertValue(
                      pTenant.getExtend().get("addressCode"), new TypeReference<>() {});
              codes.add(item.get("code").asText());
              List<String> addresses =
                  objectMapper.convertValue(
                      pTenant.getExtend().get("addressText"), new TypeReference<>() {});
              addresses.add(item.get("name").asText());
              return this.initializingTenant(
                      VillagesData.builder()
                          .codes(codes)
   ...
   * 创建租户,存在修改
   *
   * <p>addressCode 地址编码 address 租户地址 pid 租户父级ID item 租户信息
   *
   * @param data 序列化数据
   * @return 创建后的租户
   */
    ...
    return this.groupManager
        .add(
            GroupRequest.of(
                tenant.getId(),
                tenant.getCode(),
                tenant.getName() + "系统管理组",
                "初始化租户的时候,系统自动创建的默认管理组.",
                1,
                SystemType.country))
        .transformDeferredContextual(
            (groupMono, contextView) -> {
              ...
              return groupMono
                  ...
                  .delayUntil(
                      group ->
                          this.systemClient
                              .authorities(systemType.name())
                              .flatMap(
                                  jsonNode -> {
                                    List<String> authorities =
                                        new ArrayList<>(
                                            List.of(jsonNode.get("authority").asText()));
                                    jsonNode
                                        .withArray("permissions")
                                        .forEach(
                                            node ->
                                                authorities.add(node.get("permission").asText()));
                                    return this.groupManager.authorizing(
                                        group.getId(),
                                        AuthorityGroupRequest.withAuthorities(
    ...
    return this.webClient
        ...
        .uri(
            uriBuilder ->
                uriBuilder
                    .scheme("https")
                    .host("static.jinqiruanjian.com")
                    .path("/countryside/config/pcas-code.json")
                    .build())
        .retrieve()
