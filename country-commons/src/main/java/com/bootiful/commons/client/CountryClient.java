...
import com.bootiful.commons.utils.*;
 ...
 * 平台访问工具类
 * {@link CountryClient#systemClient} 访问平台工具服务
...
public class CountryClient extends AbstractClient {
    ...
    private final static String OAUTH_SERVICE_BASE_URL = "http://bootiful-oauth";
    private final static String SYSTEM_SERVICE_BASE_URL = "http://bootiful-system";
    ...
    public Mono<SimplerSecurityDetails> loadSecurity(String username) {
        return this.oauthClient.get()
                .uri(uriBuilder -> uriBuilder.path("/oauth2/v1/load/security/{username}")
                        .build(username))
                .retrieve()
    ...
    public Flux<Configuration> loadConfiguration(Configuration configuration) {
        ...
        return this.systemClient.get()
                .uri(uriBuilder -> uriBuilder.path("tenant/configuration/v1/search").queryParams(params).build())
                .retrieve()
