...
public class WxMaUserController {
    ...
    public Mono<Object> authorize(@PathVariable String appId, String code, ServerWebExchange exchange) {
        ...
        try {
            WxMaJscode2SessionResult session = WxMaConfiguration.getMaService(appId)
                    .getUserService().getSessionInfo(code);
            return this.maUserService.anonymousLogin(session, exchange).map(token ->
                    Map.of("authentication", token, "jscode2Session", session));
    ...
    @PostMapping("login")
    public Mono<AuthenticationToken> login(@PathVariable String appId, @Valid @RequestBody WxRequest regRequest,
    ...
    public Mono<WxMaUserInfo> info(@PathVariable String appId, String sessionKey,
        ...
        log.debug("请求[AppId]: {}", appId);
        // 用户信息校验
        if (!WxMaConfiguration.getMaService(appId).getUserService().checkUserInfo(sessionKey, rawData, signature)) {
            return Mono.defer(() ->
                    Mono.error(RestServerException.withMsg(1403, "用户信息验证失败!")));
        }

        // 解密用户信息
        WxMaUserInfo userInfo = WxMaConfiguration.getMaService(appId).getUserService()
    ...
    public Mono<WxMaPhoneNumberInfo> phone(@PathVariable String appId, @RequestBody Map<String, String> params) {
        ...
        try {
            WxMaPhoneNumberInfo wxMaPhoneNumberInfo = WxMaConfiguration.getMaService(appId)
                    .getUserService().getPhoneNoInfo(params.get("sessionKey"),
                            params.get("encryptedData"), params.get("iv"));
