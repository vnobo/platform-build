package com.bootiful.commons.client;

import com.bootiful.commons.annotation.exception.ClientRequestException;
...
/**
 * com.bootiful.commons.client.AbstractClient
 * 访问服务基础工具类,使用 webclient 取派生.
 * 使用此基础类统一系统化错误处理
 * 统一调度优化查询
 *
...
public abstract class AbstractClient implements InitializingBean {
    ...
    @Autowired
    public final void initWebClient(WebClient.Builder builder,
                                    ReactorLoadBalancerExchangeFilterFunction loadBalancer) {
        this.webClient = builder.filter(new ExceptionFilterFunction())
                .filter(new SecuritySessionHandlerFilterFunction()).filter(loadBalancer).build();
    }
        ...
        public Mono<ClientResponse> filter(ClientRequest request, ExchangeFunction next) {
            ...
            return next.exchange(request).flatMap(response -> {
                if (response.statusCode().isError()) {
                    return response.createException().flatMap(ex -> Mono.defer(() ->
                            Mono.error(ClientRequestException.withMsg("[" + serviceId + "]服务未准备就绪! 信息: "
                                    + ex.getMessage()))));
                } else {
    ...
    /**
     * 内部client安全认证设置 {@link com.bootiful.commons.annotation.CustomServerAuthenticationEntryPoint}
     * 设置会话seesion 向后传播
     */
    @Log4j2
                            ...
                            .headers(httpHeaders -> {
                                ...
                                if (oldHttpHeaders.containsKey("X-Auth-Token")) {
                                    httpHeaders.add("X-Auth-Token",
                                            Objects.requireNonNull(oldHttpHeaders.get("X-Auth-Token")).get(0));
                                }
                                if (oldHttpHeaders.containsKey("login-system")) {
                                    httpHeaders.add("login-system",
                                            Objects.requireNonNull(oldHttpHeaders.get("login-system")).get(0));
                                }
                            })
