...
import com.bootiful.gateway.client.SystemClient;
...
public class SecurityController {
    ...
    public Mono<AuthenticationToken> phoneLogin(@Valid @RequestBody LoginRequest loginRequest,
        ...
        return systemClient.smsCheck(loginRequest).filter(res -> res)
                ...
                .doOnNext(res -> exchange.getSession()
                        .doOnNext(webSession -> webSession.setMaxIdleTime(Duration.ofDays(2))).subscribe())
                .flatMap(res -> detailsService.appLogin(loginRequest
                        .system(SecurityTokenHelper.systemForHeader(exchange))))
                .flatMap(authentication -> SecurityTokenHelper
                        .authenticationTokenMono(exchange, authentication))
    ...
    public Mono<AuthenticationToken> refresh(ServerWebExchange exchange, Authentication authentication) {
        ...
        return this.userDetailsService.findByUsername(authentication.getName())
                .map(userDetails -> new SecurityContextImpl(new UsernamePasswordAuthenticationToken(userDetails,
    ...
    public Mono<AuthenticationToken> changePassword(@Pattern(regexp = "^.*(?=.{6,})(?=.*\\d)(?=.*[A-Z])(?=.*[a-z]).*$",
            message = "登录密码[password]必须为,最少6位,包括至少1个大写字母，1个小写字母，1个数字.") String password,
        ...
        return this.detailsService.updatePassword((UserDetails) authentication.getPrincipal(), password)
                .map(userDetails -> new UsernamePasswordAuthenticationToken(userDetails, authentication.getCredentials(),
                        userDetails.getAuthorities()))
                .flatMap(authenticationToken -> SecurityTokenHelper.authenticationTokenMono(exchange, authenticationToken))
