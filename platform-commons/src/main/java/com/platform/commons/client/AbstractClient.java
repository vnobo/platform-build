...
public abstract class AbstractClient implements InitializingBean {
  ...
  public final void initWebClient(
      WebClient.Builder builder, ReactorLoadBalancerExchangeFilterFunction loadBalancer) {
    this.webClient =
        builder
            .filter(new ExceptionFilterFunction())
   ...
   */
  public <T> Flux<T> get(
      String path, Map<String, String> queryParams, Class<T> responseType, Object... uriVariables) {
                      ...
                      .flatMap(
                          ex ->
                              Mono.defer(
                                  () ->
                                      Mono.error(
                                          ClientRequestException.withMsg(
                                              "["
                                                  + serviceId
                                                  + "]服务未准备就绪! 信息: "
                                                  + ex.getMessage()))));
    ...
    public Mono<ClientResponse> filter(ClientRequest request, ExchangeFunction next) {
      return Mono.deferContextual(
          contextView -> {
            ...
            Optional<ServerWebExchange> exchangeOpt =
                contextView.getOrEmpty(ServerWebExchange.class);
            if (exchangeOpt.isPresent()) {
              ...
              ClientRequest clientRequest =
                  ClientRequest.from(request)
                      ...
                      .attributes(
                          stringObjectMap -> stringObjectMap.putAll(exchange.getAttributes()))
                      .build();
