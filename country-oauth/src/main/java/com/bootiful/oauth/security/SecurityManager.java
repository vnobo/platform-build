...
import com.bootiful.commons.security.LoginSecurityDetails;
import com.bootiful.commons.security.SecurityDetailsTenant;
...
import com.bootiful.oauth.security.tenant.member.MemberTenantManager;
import com.bootiful.oauth.security.tenant.member.MemberTenantOnly;
import com.bootiful.oauth.security.tenant.member.MemberTenantRequest;
import com.bootiful.oauth.security.user.User;
import com.bootiful.oauth.security.user.UserBinding;
...
public class SecurityManager extends BaseAutoToolsUtil {
    ...
    public Mono<LoginSecurityDetails> register(RegisterRequest request) {
        ...
        return this.userManager.register(request.toUserRequest())
                .flatMap(user -> authoritiesFunction.apply(user)
                        .map(authorities -> LoginSecurityDetails.of(user.getUsername(),
                                user.getPassword(), user.getEnabled()).authorities(authorities)))
    ...
    public Mono<LoginSecurityDetails> login(String username) {
        ...
        return userMono.flatMap(user -> authoritiesFunction.apply(user)
                .map(authorities -> LoginSecurityDetails.of(user.getUsername(), user.getPassword(), user.getEnabled())
    ...
    public Mono<SimplerSecurityDetails> loadSecurity(String username) {
        ...
        Mono<SimplerSecurityDetails> securityDetailsMono = this.userManager.loadByUsername(username)
                .map(user -> SimplerSecurityDetails.of(user.getId(), user.getUsername(), user.getName(),
                        Validator.isChinese(user.getName()) && IdcardUtil.isValidCard(user.affirmIdCard()) ? 1 : 0));

        Function<SimplerSecurityDetails, Flux<SecurityDetailsTenant>> userTenantFunction = securityDetails ->
                this.memberTenantManager.search(MemberTenantRequest.withUserId(securityDetails.getUserId()))
                        .map(tenantOnlyName -> SecurityDetailsTenant.of(tenantOnlyName.getTenantId(),
                                tenantOnlyName.getTenantCode(), tenantOnlyName.getTenantName(), tenantOnlyName.getIsDefault(),
                                tenantOnlyName.getUserId(), tenantOnlyName.getUserName(), tenantOnlyName.getTenantExtend()));


        return securityDetailsMono.flatMap(securityDetails -> userTenantFunction.apply(securityDetails)
                .distinct().collectList()
                .map(tenants -> securityDetails.tenants(tenants.parallelStream().collect(Collectors.toSet()))));
    ...
    private Mono<String[]> authorities(long userId) {
        return this.getAuthorities(userId).concatWith(this.getGroupAuthorities(userId))
                .distinct().collectList()
                .map(listAuth -> listAuth.parallelStream().map(SimpleAuthority::getAuthority)
    ...
    private Flux<SimpleAuthority> getAuthorities(long userId) {
        return this.userManager.loadAuthorities(userId)
                .cast(SimpleAuthority.class);
    }

    /**
     * 根据用户ID 获取用户组权限
     * 如果流存在SYSTEM 根据系统类型过滤
     *
     * @param userId 用户ID
     * @return 用户所有组权限
     */
    ...
    public Mono<SimplerSecurityDetails> tenantCut(MemberTenantRequest memberTenantRequest) {
        return this.memberTenantManager.operation(memberTenantRequest).delayUntil(this::updateUserTenant)
                .map(memberTenant -> SimplerSecurityDetails.of(memberTenant.getUserId(), null,
                        memberTenant.getUserName(), 1));
    }
    ...
    private Mono<Integer> updateUserTenant(MemberTenantOnly memberTenant) {
        return entityTemplate.update(User.class).matching(Query.query(Criteria.where("userId")
                        .is(memberTenant.getUserId())))
